<template>
  <div id="sidebar-app">
    ...
    >
      ...
      <!-- 菜单 (el-submenu) index 为子应用名称，子菜单 (el-menu-item) index 为路由地址 -->
...
export default {
  ...
  methods: {
    // 根据 url 地址获取选中菜单
    getActiveIndex () {
      // location.pathname 的值通常为：/main-angular11/app-vue2/page2，我们只取`/app-vue2/page2`
      ...
      // 兼容 child-vite 和 child-react17 子应用，因为它们是 hash 路由
      if (
    ...
    select (index, indexPath) {
      if (this.microAppData) {
        // 因为 child-vite 和 child-react17 子应用是 hash 路由，所以需要传递 hash 值
        let hash = null
